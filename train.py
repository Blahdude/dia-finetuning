#!/usr/bin/env python3
"""Universal training script for audio-text pairs."""
...
def get_training_config(epochs: int = 300, learning_rate: float = 3e-4, output_dir: str = "checkpoints"):
    """Get training configuration with sensible defaults."""
    return TrainConfig(
        epochs=epochs,
        batch_size=1,               # Single sample overfitting
        grad_accum_steps=1,         # Single sample needs immediate updates
        learning_rate=learning_rate,
        warmup_steps=min(epochs // 3, 300),  # 1/3 of epochs or 300, whichever is smaller
        ...
        run_name=f"audio_overfit_{Path(output_dir).name}",
...
def main():
    parser = argparse.ArgumentParser(description="Train Dia model on audio-text pair")
    ...
    print(f"Training Dia model on audio-text pair:")
    ...
    wandb.init(
        ...
        name=f"instrumental-151s-lr{args.lr}",
        config={
            "learning_rate": args.lr,
            "epochs": args.epochs,
